...
> Returned **in-conversation** by the Investigator (from `/discussion` or `/create-brief`) — **not a file**.
> Feeds the bug brief's Root cause + Suggested resolution path sections.
