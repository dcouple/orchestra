...
> Produced by the postmortem workflow on an autonomous-pipeline run — the Run operations half always, the outcome
...
type: postmortem
item: <id>
pr: <url or number of the pipeline PR — "none" if the failure predates a PR>
...
`Harness usage events — "unknown" only after checking`
