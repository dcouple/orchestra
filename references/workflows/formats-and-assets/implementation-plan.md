...
> Produced by the autonomous pipeline's plan stage from the work item. Saved
> as `./tmp/<id>/plan.md`.
...
requested_lanes: <dual | single — only when the execution environment signals fallback>
effective_lanes: <single — only when the execution environment signals fallback>
runtime_fallback: <preferred route -> fallback route — only when fallback occurred>
fallback_cause: <environment-classified cause — only when fallback occurred>
phase: <n | —>
