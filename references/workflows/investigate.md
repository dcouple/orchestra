# Investigate workflow contract

Use one investigator to reproduce the defect, isolate its cause, and report the evidence. This workflow is the human-facing front door; the `investigator` role owns the diagnostic work.

## 1. Frame the Defect

Build a compact self-contained brief from the defect input and conversation:
...
## 3. Request the Single Investigator

The harness adapter must request and await exactly one `investigator`. Pass a
...
Do not request a separate `code-researcher` for the same investigation. The investigator owns reproduction, code tracing, history, runtime evidence, and root-cause isolation end to end. A second request is justified only when the first finding explicitly names missing evidence that has since become available.

If the investigator role is unavailable, follow the shared method directly in
the current session and disclose that fallback.
