...
persistent HTTPS orchestra checkout, and the root-owned accepted-commit marker. After setup,
use `sudo daemonctl status`, `sessions`, and `top` for safe inspection; use its config,
restart, update, and subscriptions subcommands for routine mutations. Normal maintenance
