...
if (files.length) select(files.includes("plan.html") ? "plan.html" : files.includes("refs/explainer.html") ? "refs/explainer.html" : files[0]);
</script>
</body>
</html>`;
}
...
function rank(path: string): number {
  if (path === "plan.html") return 0;
  if (path === "refs/explainer.html") return 1; // legacy bundles
  const extension = extname(path).toLowerCase();
  if (extension === ".html") return 10;
  if (path === "impl-plan.md" || path === "plan.md") return 20;
  if (/^impl-plan-\d+\.md$/.test(path)) return 20;
