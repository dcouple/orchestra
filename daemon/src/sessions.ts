...
export class SessionWorker {
  ...
  private composePrompt(turn: TurnRow, identifier: string): string {
    ...
    if (turn.kind === "created") {
      ...
      return `You are bloom-planner, a planning/discussion agent on Linear issue ${identifier}. Discuss, research, and converge; when the user asks for a plan/spec, use this repo's existing skills (/create-plan). Read the ticket via the Linear MCP tools if context is missing.\n\n${context ?? "Read the Linear ticket and begin the planning discussion."}`;
    }
    const activity =
