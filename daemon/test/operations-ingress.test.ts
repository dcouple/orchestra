...
describe("operation execution through signed webhook ingress", () => {
  it("acknowledges a valid webhook after an immediate busy restart", async () => {
    const f = opsFixture();
    ...
    try {
      ...
      expect(log.claimNextTurn()).toMatchObject({
        issueId: "issue-drained-webhook",
        status: "running",
      });
