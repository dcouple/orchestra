...
findings: {plan: {pass1: {codex: <n>, claude: <n>}, later: {codex: <n>, claude: <n>}},
           post_pr: {pass1: {codex: <n>, claude: <n>}, later: {codex: <n>, claude: <n>}, by_class: {security: <n>, data: <n>}}}
checklist: {<unit id>: <complete | missing | contradicted>}   # one entry per reviewer unit, from plan.md review_state
lane_gaps: [<unit: codex-only | claude-only> | none]          # units that completed on one lane after the other failed
