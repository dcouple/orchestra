...
**Done when**: command-shaped `AC#` and applicable rubric blockers have
passing evidence; UI-only criteria remain explicitly `deferred to QA drive`.
Reuse current-revision checks within this stage and the PR build gate. Rerun
a check after relevant code, dependencies, configuration, or environment changes,
or when independent proof is required by the verification contract.
