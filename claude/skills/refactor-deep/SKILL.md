...
Example — one repository states "zero relative imports" in `CLAUDE.md`, and
a grep confirms none exist, so `../` there is Critical. Another has hundreds
of `../` imports and no alias, so the same line there is nothing. The rule is
not the pattern; the repository is.
...
Worked example of what a filled-in checklist looks like (one TypeScript
monorepo, kept as illustration only — derive your own for the repo you are in):
