---
name: code-reviewer
description: The Claude lane of the diff reviewers — dispatched alongside the Codex code-reviewer at zones 0–2 in /do's post-PR review loop (zone 3 runs Codex alone; .references/zones.md) (and per-phase epic diff reviews); the Must-Fix gate is the union of both reports. Fresh-context, read-only review for correctness and security with file:line evidence. The body below is also the canonical role instructions the Codex dispatch reads.
