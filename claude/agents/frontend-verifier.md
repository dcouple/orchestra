...
description: Verify running-app criteria and PR journeys with browser or mobile evidence, or reproduce a reported failure.
...
- **Verify**: execute supplied acceptance criteria and a rubric, with evidence
  for each. In `/do`, UI verification belongs to the post-PR QA stage.
...
Read `.references/artifact-storage.md`. Keep captures and manifests at their
